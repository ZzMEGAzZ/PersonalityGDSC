<script lang="ts">
	...
	import { mode, name, lang } from '../stores';
...
<div class="relative w-full h-full flex flex-col items-center justify-center space-y-5">
	...
	<button class={`${$name.length < 1 && 'hidden'} text-2xl tablet:text-3xl py-1 px-6 tablet:py-2 tablet:px-8 bg-MainGreen rounded-xl text-MainWhite m-2`} on:click={() => ($mode = 'consent')}>{$lang === 'en' ? 'Next' : 'ถัดไป'}</button>
</div>
